...
def build_history_with_loans(
    fixtures_df: pd.DataFrame, picks_long: pd.DataFrame, loans: list[dict]
) -> pd.DataFrame:
    """Like build_history, but redirects group-stage match points for loaned teams
    to their temporary receivers. Bonus/elimination points stay with original owner."""
    columns = ["date", "persona", "total"]
    if fixtures_df.empty or picks_long.empty:
        return pd.DataFrame(columns=columns)
    ...
    for date, day_fixtures in finished.groupby("date"):
        for _, f in day_fixtures.iterrows():
            ...
            for team, result in ((f["home_team"], home_result), (f["away_team"], away_result)):
                persona = team_to_persona.get(team)
                if persona:
                    persona = loan_map.get((f["fixture_id"], team), persona)
                    totals[persona] += MATCH_POINTS[result]
